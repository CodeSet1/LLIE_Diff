...
class myImageFloder(data.Dataset):
    def __init__(self,root,transform = None,target_transform = None,crop=True,resize=True,crop_size=480,resize_size=256):
        ...
        for img_name in os.listdir(os.path.join(root,'low')):
            if img_name !='.' and img_name !='..':
                imgin_names.append(os.path.join(root,'low',img_name))
                
        for img_name in os.listdir(os.path.join(root,'high')):
            if img_name !='.' and img_name !='..':
                imgout_names.append(os.path.join(root,'high',img_name))
        imgin_names.sort()
        imgout_names.sort()
    ...
    def __getitem__(self,index):
        ...
        if "LOL" in self.root and not self.crop and not self.resize:
            [w, h] = img1.size
            if w != 720 or h != 480:
